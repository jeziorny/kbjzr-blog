...
const outputFilePath = path.join(__dirname, '../public/posts-meta.json');
...
function generatePostsMeta() {
  ...
  files.forEach(file => {
    if (file.endsWith('.md')) {
      ...
      posts.push({
        ...
        file: `posts/${file}` // Path relative to public directory
      });
    }
  });
  ...
  console.log('Generated public/posts-meta.json');
}
